...
export function SponsorRail({
  ...
  const railClass = variant === "dark" ? styles.sponsorRail : styles.sponsorRailLight;
  const cardClass = variant === "dark" ? styles.sponsorCard : styles.sponsorCardLight;

  return (
    <section className={railClass} aria-label="Sponsor integrations">
      {sponsors.map((sponsor) => {
        ...
        return (
          <article className={cardClass} data-live={live || undefined} key={sponsor.name}>
            <img src={`https://www.google.com/s2/favicons?sz=64&domain=${sponsor.domain}`} alt="" />
            <div>
              <strong>{sponsor.name}</strong>
              <span>{live ? "active" : ready ? "armed" : "off"}</span>
            </div>
          </article>
        );
      })}
